...
def run_step2_recebiveis(
    ...
    if prov == "FARMACIASAPP":
        ...
        for rr in rows_db:
            ...
            valor_pago_loja = s_info.get("valor_pago_loja")
            valor_pago_loja = valor_pago_loja if isinstance(valor_pago_loja, Decimal) else (parse_decimal(valor_pago_loja) or Decimal("0"))

            subsidio_d = parse_decimal(subsidio) if subsidio is not None else pd.NA
            ...
            taxa_esp = (total_compra * (perc_esp / Decimal("100"))).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
            repasse_esp = (total_compra - taxa_esp).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
            ...
            out.append({
                ...
                "valor_subsidio": subsidio_d if not pd.isna(subsidio_d) else pd.NA,
                "valor_total_compra": total_compra,
